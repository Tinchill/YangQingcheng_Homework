...
​        训练模型，每隔10个epoch保存一次模型至 **./pretrained** 路径下。
...
​        用预训练模型中误差最小的一个，将对 **./detection/test** 下图片所预测的目标边界框、置信度等结果可视化。
...
​        ○ 根据情况修改训练所用设备变量 **_device** 为 **'cpu'/'cuda'**。（第17行）
...
​        **○ 提升方案B**仍使用 **Adam** 迭代优化器，设置学习步长为 **0.001**，权重衰减为 **0.0005**；
